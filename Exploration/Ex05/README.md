...
# 6-1에서 예시 답안을 보면 vocab 변수에 '<PAD>', '<BOS>', '<UNK>', '<UNUSED>' 이 토큰들이 ''로 대체되어 있는 걸 확인할 수가 있는데 Mecab은 내부적으로만 BOS 등을 사용하고 반환 토큰에는 해당 내용이 없어서 그런지는 몰랐었다. Mecab에 대해 더 알아봐야 할 것 같다.
```
